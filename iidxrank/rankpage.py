...
"""
compiles player object into player data
"""
def get_player_data(pobj,ranktable):
    ...
    prs = models.PlayRecord.filter(user=pobj,song__in=songs).all()
    for pr in prs:
        notecnt = pr.song.notecount
        song = pr.song
        if (notecnt == None):
            rate = 0
        else:
            rate = float(pr.playscore) / pr.song.notecount
        music = {
            'pkid': song.pk,
            'rate': rate,
            'rank': iidx.getrank(rate),
            'clear': pr.playclear,
            'clearstring': iidx.getclearstring(pr.playclear),
            'data': {
